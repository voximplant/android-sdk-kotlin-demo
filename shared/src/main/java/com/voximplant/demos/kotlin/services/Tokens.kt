...
class Tokens(private val context: Context) {
    ...
    val state: TokensState
        get() {
            if (!tokensValid) {
                return TokensState.Expired
            }
            if (accessExpired) {
                return TokensState.NeedsRefresh
            }
            return TokensState.Valid
        }

    private val tokensExist: Boolean
        get() =
            ACCESS_TOKEN.getStringFromPrefs(context) != null && REFRESH_TOKEN.getStringFromPrefs(
                context
            ) != null
    ...
    private val tokensValid: Boolean
        get() = if (tokensExist && !refreshExpired) {
            !refreshExpired
        } else {
            false
        }
